...
function powerUp(player)
    for index, monster in pairs(player.Field.MonsterFields) do
        if monster ~= nil and (monster.MonsterType == 1 or monster.MonsterType == 6 or
                monster.MonsterType == 16) then
...
function processWhenSummon(targetID, player)
    ...
    if target ~= nil and (target.MonsterType == 1 or target.MonsterType == 6 or
            target.MonsterType == 16) then
